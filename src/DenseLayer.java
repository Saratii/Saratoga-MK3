    ...
    public Matrix backward(Matrix previousDerivatives, int threadIndex) {
        ...
        for (int i = 0; i < inputs[threadIndex].size; i++) {
            ...
            for (int j = 0; j < NUM_NODES; j++) {
                passedOnDerivatives.matrix[0][i] += previousDerivatives.matrix[0][j]
                        * weights.matrix[0][j * inputs[threadIndex].size + i];
                weightGradient[threadIndex].matrix[0][j * inputs[threadIndex].size
                        + i] += previousDerivatives.matrix[0][j] * inputs[threadIndex].matrix[0][i];
            }
        }
