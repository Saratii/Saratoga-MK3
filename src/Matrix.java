...
public class Matrix {
    ...
    public static BufferedImage makeSquare(BufferedImage image) {
        if(image.getHeight() > image.getWidth()) {
            BufferedImage im = image.getSubimage(0, image.getHeight() / 2 - image.getWidth() / 2, image.getWidth(),
                    image.getWidth());
            return im;
        } else {
            BufferedImage im = image.getSubimage(image.getWidth() / 2 - image.getHeight() / 2, 0, image.getHeight(),
                    image.getHeight());
            return im;
        }
    }
        ...
        for (int i = 0; i < rows; i++) {
            for (int j = 0; j < matrixB.cols; j++) {
                ...
                for (int k = 0; k < cols; k++) {
                    result.matrix[0][i * matrixB.cols + j] += matrix[0][i * cols + k]
                            * matrixB.matrix[0][k * matrixB.cols + j];
                }
            }
        }
        return result;
